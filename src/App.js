...
class App extends Component {
  state = {
    ...
    score: 0,
    shuffledImagesList: [imagesList[0].imageUrl],
  ...
  updateScore = () => {
    const {shuffledImagesList} = this.state

    this.setState({score: shuffledImagesList.length})
  }

  onClickImage = thumbnailUrl => {
    const {shuffledImagesList, id} = this.state
    const shuffledImage = imagesList.sort(() => Math.random() - 0.5)
    const isEqual =
      shuffledImagesList.includes(id) === thumbnailUrl.includes(id)

    if (!isEqual) {
      this.renderScoreCard()
    } else {
      this.setState(prevState => ({
        shuffledImagesList: [...prevState.shuffledImagesList, shuffledImage],
      }))
      this.updateScore()
    }
  }
      ...
      <div className="app-container">
        <ul>
          <div className="name-score-container">
            <Navbar score={score} timeElapsedInSeconds={timeElapsedInSeconds} />
          </div>
        </ul>
        <div className="image-shuffle-container">
          <img src={shuffledImagesList} className="shuffle-image" alt="match" />
        </div>
